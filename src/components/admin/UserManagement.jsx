...
const UserManagement = ({ allowedRoles = ['student'], canApprove = false, title = 'Gestion des États', showRoleFilter = true, onViewDetails }) => {
    ...
    const filteredUsers = users.filter(u => {
        ...
        // Check if user role is in allowedRoles prop (to hide other admins/roles from restricted views)
        // If allowedRoles includes 'student', we only show students?
        // Actually, allowedRoles usually dictates what we can Manage.
        // But for viewing, maybe we want to see only those roles too?
        // Yes, if I am Responsable managing Students, I shouldn't see Admins.
        const roleScopeMatch = allowedRoles.includes(u.role);

        // Filter dropdown
        const roleFilterMatch = filterRole === 'all' || u.role === filterRole;

        return statusMatch && roleScopeMatch && roleFilterMatch;
    });
                                    ...
                                    <td className="p-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            {canApprove && u.status === 'pending' && (
                                                <>
                                                    <button title="Approuver" className="text-green-600 hover:bg-green-50 p-1 rounded" onClick={() => handleStatusChange(u._id, 'active')}>✅</button>
                                                    <button title="Rejeter" className="text-red-600 hover:bg-red-50 p-1 rounded" onClick={() => handleStatusChange(u._id, 'rejected')}>❌</button>
                                                </>
                                            )}
                                            {u.status === 'active' && (
                                                <button title="Suspendre" className="text-orange-600 hover:bg-orange-50 p-1 rounded" onClick={() => handleStatusChange(u._id, 'suspended')}>⏸️</button>
                                            )}
                                            {u.status === 'suspended' && (
                                                <button title="Réactiver" className="text-blue-600 hover:bg-blue-50 p-1 rounded" onClick={() => handleStatusChange(u._id, 'active')}>▶️</button>
                                            )}
                                            <button
                                                title="Modifier"
