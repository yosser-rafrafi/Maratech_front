...
const StudentFormationDetails = () => {
    ...
    useEffect(() => {
        const fetchFormationDetails = async () => {
            try {
                // Fetching all dashboard data to find the specific formation
                // In a optimized scenario, we would have a specific endpoint /student/formation/:id
            ...
            <main className="flex-1 ml-64 p-8">
                <header className="mb-10">
                    <Link to="/student-formations" className="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors mb-4">
                        ...
                        Back to My Formations
                    </Link>
                    <h1 className="text-3xl font-bold tracking-tight mb-2">
                        {formation.title}
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400">
                        Detailed progress and level status.
                    </p>
                </header>
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700">
                            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Course Levels</h2>
                        </div>
                                    ...
                                    <div className="flex items-center gap-4 flex-1">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                                {lvl.attended} / {lvl.total} sessions completed
                                            </p>
                                            <span className={`inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase ${lvl.status === 'validated'
                                                ...
                                                {lvl.status === 'in_progress' ? 'En Cours' : lvl.status === 'validated' ? 'Validé' : 'Bloqué'}
                                            </span>
                                        </div>
                                    </div>
